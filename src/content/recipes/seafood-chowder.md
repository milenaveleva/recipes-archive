---
title: Seafood Chowder
description: test now
imageUrl: https://www.recipetineats.com/tachyon/2025/09/Seafood-chowder_9.jpg
source:
  name: recipetineats.com
  url: https://www.recipetineats.com/seafood-chowder/
servings: 4
prepTime: PT20M
cookTime: PT20M
totalTime: PT40M
cuisine: Western
course: Mains
ingredients:
  - raw: 650g/ 1.3lb seafood marinara mix or mixed fresh seafood - fish (, squid, prawns, cooked mussel meat (Note 1))
    quantity: 650
    unit: g
    item: / 1.3lb seafood marinara mix or mixed fresh seafood - fish (
    note: squid, prawns, cooked mussel meat (Note 1))
    grams: 650
    fdcId: 175168
    matchConfidence: low
  - raw: 50g (3 tbsp) unsalted butter
    quantity: 50
    unit: g
    item: (3 tbsp) unsalted butter
    grams: 50
    fdcId: 173430
    matchConfidence: low
  - raw: 100g/ 4oz streaky bacon (, chopped into 1.5cm / 1/2" squares)
    quantity: 100
    unit: g
    item: / 4oz streaky bacon
    note: ', chopped into 1.5cm / 1/2" squares'
    grams: 100
    matchConfidence: none
  - raw: 2 cloves garlic (, finely minced)
    quantity: 2
    unit: cloves
    item: garlic
    note: ", finely minced"
    grams: 6
    fdcId: 169230
    matchConfidence: high
  - raw: 1/2 cup chardonnay (or other dry white wine (can skip))
    quantity: 0.5
    unit: cup
    item: chardonnay (or other dry white wine (can skip))
    milliliters: 118.294
    matchConfidence: none
  - raw: 1/3 cup flour (, plain/all-purpose (Note 3 for gluten-free))
    quantity: 0.333
    unit: cup
    item: flour (
    note: plain/all-purpose (Note 3 for gluten-free))
    grams: 41.8
    milliliters: 78.783804
    fdcId: 169761
    matchConfidence: high
  - raw: 1 litre (4 cups) chicken stock/broth, low sodium OR homemade fish stock ((avoid store-bought fish stock) (Note 2))
    quantity: 1
    item: litre (4 cups) chicken stock/broth
    note: low sodium OR homemade fish stock ((avoid store-bought fish stock) (Note 2))
    fdcId: 171534
    matchConfidence: low
  - raw: 2 medium carrots (, peeled, cut into 7mm / 1/3" pieces (~1 1/2 cups))
    quantity: 2
    unit: medium
    item: carrots (
    note: peeled, cut into 7mm / 1/3" pieces (~1 1/2 cups))
    fdcId: 170393
    matchConfidence: high
  - raw: 2 large potatoes ((any type fine), peeled, cut into 1cm / 0.2" cubes (~2 1/2 cups))
    quantity: 2
    unit: large
    item: potatoes ((any type fine)
    note: peeled, cut into 1cm / 0.2" cubes (~2 1/2 cups))
    fdcId: 170026
    matchConfidence: low
  - raw: 1 cup thickened cream / heavy cream ((sub milk for lighter option))
    quantity: 1
    unit: cup
    item: thickened cream / heavy cream ((sub milk for lighter option))
    milliliters: 236.588
    fdcId: 746782
    matchConfidence: low
  - raw: 1 cup corn ((frozen or canned, drained))
    quantity: 1
    unit: cup
    item: corn ((frozen or canned
    note: drained))
    milliliters: 236.588
    fdcId: 170052
    matchConfidence: medium
  - raw: 2 tsp fish sauce
    quantity: 2
    unit: tsp
    item: fish sauce
    milliliters: 9.85784
    fdcId: 175168
    matchConfidence: low
  - raw: Pinch white pepper ((substitute black pepper))
    unit: Pinch
    item: white pepper ((substitute black pepper))
    fdcId: 170931
    matchConfidence: medium
  - raw: 3 tbsp chives or parsley (, finely chopped, for garnish)
    quantity: 3
    unit: tbsp
    item: chives or parsley
    note: ", finely chopped, for garnish"
    milliliters: 44.3604
    matchConfidence: none
  - raw: Crusty bread ( or better yet, garlic bread!)
    item: Crusty bread
    note: or better yet, garlic bread!
    matchConfidence: none
nutrition:
  perServing:
    energyKcal: 465
    energyKj: 1944
    protein_g: 37.2
    fat_g: 30.3
    satFat_g: 11.4
    carbs_g: 8.5
    fiber_g: 0.3
    sugar_g: 0.1
    sodium_mg: 180
    availableCarb_g: 8.2
  glycemic:
    gi: 75
    gl: 6
    giBand: high
    glBand: low
    gi_source: Atkinson 2021 GI tables (carb-weighted composite estimate)
  nutriScore:
    grade: C
    points: 3
    category: general
    version: "2023"
  inflammation:
    score: -1.6
    band: anti-inflammatory
    method: ingredient-tag v1
  computedAt: "2026-06-20"
  dataSources:
    - USDA FoodData Central
    - Atkinson 2021 GI tables
    - Nutri-Score 2023
    - Inflammation index (ingredient-tag v1)
createdAt: "2026-06-19"
updatedAt: "2026-06-20"
---

## Method

1. Cook bacon in butter, remove. Sauté garlic, deglaze with white wine, make roux. Add stock, simmer potato, carrots and bacon 10 min. Add cream, corn, fish sauce, pepper, raw seafood - simmer 3 min. Stir in cooked seafood, taste for salt. Serve!
2. Marinara mix - Separate cooked seafood (usually mussels, sometimes prawns/shrimp) from raw seafood. We put them in at different times.
3. Cut any large pieces of fish into 2.5cm / 1" cubes.
4. Bacon - In a heavy-based pot, melt the butter over medium heat. Add bacon and cook for 3 - 4 minutes until the edges are light golden. Use a slotted spoon to remove into bowl, leaving fat in the pot.
5. Deglaze - Add the garlic and cook for 10 seconds (don't let it brown). Add the wine, simmer rapidly on high for 3 minutes, scraping the base to loosen any golden bacon bits, until the wine is mostly evaporated.
6. Roux - Reduce heat down to medium, add the flour and mix for 1 minute ("roux").
7. Broth - While stirring, pour in about 1 cup of the stock and mix to dissolve the roux - it will thicken into a paste. Add the remaining stock and stir well until lump free (use a whisk if needed).
8. Simmer 10 minutes - Increase heat to high, bring to the boil, add the carrots, potatoes and bacon. Lower heat to medium and simmer for 10 - 12 minutes, or until the carrots are just about done.
9. Simmer raw seafood 3 minutes - Add the cream, fish sauce, pepper, corn and raw seafood. Simmer for 3 minutes until the fish is just cooked (test - flesh should flake).
10. Cooked seafood - Stir in cooked seafood, taste and add more salt if needed (I don't).
11. Serve - Ladle into bowls, sprinkle with chives. Serve with warm crusty bread for dunking!
