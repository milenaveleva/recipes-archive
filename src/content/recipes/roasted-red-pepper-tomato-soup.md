---
title: Roasted Red Pepper & Tomato Soup
description: This vibrant soup is as rich in flavor as it is in color, and it’s very straightforward to make. Throw everything in to roast and then simply blend it into this inviting soup. This is a great recipe to make fresh or in advance for leftovers—it tastes fantastic either way!
image: ./images/roasted-red-pepper-tomato-soup.jpg
imageUrl: https://www.medicalmedium.com/blog-images-optimised/roasted-red-pepper-and-tomato-soup.jpg
source:
  name: medicalmedium.com
  url: https://www.medicalmedium.com/blog/roasted-red-pepper-and-tomato-soup
servings: 4
ingredients:
  - raw: 1 lb red bell peppers (roughly chopped)
    quantity: 1
    unit: lb
    item: red bell peppers
    note: roughly chopped
    grams: 453.6
    fdcId: 2258590
    matchConfidence: high
  - raw: 1 lb plum tomatoes
    quantity: 1
    unit: lb
    item: plum tomatoes
    grams: 453.6
    fdcId: 321360
    matchConfidence: medium
  - raw: 1 cup onion (diced)
    quantity: 1
    unit: cup
    item: onion
    note: diced
    grams: 160
    milliliters: 236.588
    fdcId: 170000
    matchConfidence: high
  - raw: 3 garlic cloves (roughly chopped)
    quantity: 3
    item: garlic cloves
    note: roughly chopped
    grams: 9
    fdcId: 169230
    matchConfidence: low
  - raw: 1/2 cup celery (chopped)
    quantity: 0.5
    unit: cup
    item: celery
    note: chopped
    grams: 60
    milliliters: 118.294
    fdcId: 169988
    matchConfidence: high
  - raw: 1 tsp dried thyme
    quantity: 1
    unit: tsp
    item: dried thyme
    grams: 0.8
    milliliters: 4.92892
    fdcId: 173470
    matchConfidence: high
  - raw: 1/2 tsp red pepper flakes
    quantity: 0.5
    unit: tsp
    item: red pepper flakes
    grams: 0.9
    milliliters: 2.46446
    fdcId: 170932
    matchConfidence: low
  - raw: 11/2 cups water
    quantity: 5.5
    unit: cups
    item: water
    grams: 1303.5
    milliliters: 1301.234
    fdcId: 174158
    matchConfidence: high
  - raw: Fresh basil (to serve)
    item: Fresh basil
    note: to serve
    grams: 5
    fdcId: 172232
    matchConfidence: high
nutrition:
  perServing:
    energyKcal: 84
    energyKj: 351
    protein_g: 2.7
    fat_g: 1
    satFat_g: 0
    carbs_g: 18.9
    fiber_g: 4.8
    sugar_g: 1.9
    sodium_mg: 27
    availableCarb_g: 14.2
  nutriScore:
    grade: A
    points: -1
    category: general
    version: "2023"
    coverage: 0.63
  inflammation:
    score: 0.2
    band: neutral
    method: fii v3
  balance:
    score: 8
    band: high
    nrf: 170.6
    version: NRF9.3
  processing:
    minimallyProcessedPct: 100
    ultraProcessedPct: 0
    band: minimally-processed
    method: NOVA (energy-weighted)
  computedAt: "2026-07-11"
  dataSources:
    - USDA FoodData Central
    - Nutri-Score 2023
    - Food Inflammation Index (composition-derived, energy-weighted); Phenol-Explorer polyphenols
    - Nutrient-Rich Foods Index (NRF9.3)
    - NOVA food classification (energy-weighted)
createdAt: "2026-07-11"
updatedAt: "2026-07-11"
---

## Method

## Method

1. Preheat oven to 200°C. Line a baking dish with parchment paper. Add the chopped peppers, tomatoes, diced onion, garlic, celery, thyme, and red pepper flakes to the baking dish. Mix well. Place in the oven and roast for 20-25 minutes, until browned and tender.
2. Remove from the oven and add to a blender together with the water.
3. Blend until smooth.
4. Pour into a pot and heat until simmering.
5. Ladle into soup bowls and garnish with fresh basil.
6. Serve immediately.
