---
title: Beetroot Salad with Vegan Feta
description: Our Festive Beetroot Salad topped with Vegan Feta Cheese and drizzled with a delicious Balsamic Vinaigrette, checks all the boxes to be on your special menus for this Holiday season.
imageUrl: https://sixhungryfeet.com/wp-content/uploads/2023/09/Beetroot-Salad-7.jpg
source:
  name: sixhungryfeet.com
  url: https://sixhungryfeet.com/beetroot-salad-with-vegan-feta/
servings: 4
prepTime: PT10M
cookTime: PT1H
totalTime: PT1H10M
cuisine: American
course: Side Dish
ingredients:
  - raw: 3 beetroots
    quantity: 3
    item: beetroots
    matchConfidence: none
  - raw: 3/4 cup walnuts and pecans
    quantity: 0.75
    unit: cup
    item: walnuts and pecans
    milliliters: 177.441
    fdcId: 170182
    matchConfidence: low
  - raw: 100 g vegan feta
    quantity: 100
    unit: g
    item: vegan feta
    grams: 100
    fdcId: 173420
    matchConfidence: low
  - raw: 1 pear
    quantity: 1
    item: pear
    grams: 140
    fdcId: 169118
    matchConfidence: high
  - raw: 1 cup rocket salad (arugula)
    quantity: 1
    unit: cup
    item: rocket salad
    note: arugula
    grams: 289
    milliliters: 236.588
    matchConfidence: none
  - raw: 1 cup spinach
    quantity: 1
    unit: cup
    item: spinach
    grams: 30
    milliliters: 236.588
    fdcId: 168462
    matchConfidence: high
  - raw: 2 tablespoon extra virgin olive oil
    quantity: 2
    unit: tablespoon
    item: extra virgin olive oil
    grams: 27
    milliliters: 29.5736
    fdcId: 171413
    matchConfidence: high
  - raw: 1 tablespoon balsamic vinegar
    quantity: 1
    unit: tablespoon
    item: balsamic vinegar
    grams: 16
    milliliters: 14.7868
    fdcId: 172241
    matchConfidence: high
  - raw: 6 leaves fresh mint
    quantity: 6
    item: leaves fresh mint
    matchConfidence: none
  - raw: 1 teaspoon Dijon mustard
    quantity: 1
    unit: teaspoon
    item: Dijon mustard
    grams: 1.2
    milliliters: 4.92892
    fdcId: 169256
    matchConfidence: medium
  - raw: pinch of salt
    unit: pinch
    item: salt
    fdcId: 173468
    matchConfidence: high
nutrition:
  perServing:
    energyKcal: 151
    energyKj: 633
    protein_g: 3.9
    fat_g: 12.2
    satFat_g: 4.3
    carbs_g: 7.3
    fiber_g: 1.3
    sugar_g: 4
    sodium_mg: 292
    availableCarb_g: 6
  nutriScore:
    grade: D
    points: 11
    category: general
    version: "2023"
  inflammation:
    score: -1.5
    band: anti-inflammatory
    method: ingredient-tag v1
  computedAt: "2026-06-23"
  dataSources:
    - USDA FoodData Central
    - Nutri-Score 2023
    - Inflammation index (ingredient-tag v1)
createdAt: "2026-06-23"
updatedAt: "2026-06-23"
---

## Method

1. Preheat the oven to 200C.
2. Cut the top and bottom of the beet, wash with water, and wrap in tinfoil paper.
3. Place on a baking tray and bake for about an hour at 200C. You may need to adjust the cooking time depending on the size of your beets. To make sure the beets are cooked, stick a knife. This should go all the way in easily.
4. Allow the beets to cool down, then peel them using your hands. Cut the beets into small wedges or slices, or your preferred shape, and reserve.
5. Combine all the dressing ingredients in a small mixing bowl. Mix well until all the ingredients are incorporated and you have a uniform sauce.
6. Finely slice the pear, and crush the nuts. You can cut into cubes the vegan feta or crumble with your hands.
7. Using a big salad bowl, combine the fresh leaves, beetroot wedges, sliced pear, and chopped nuts. Add the vinaigrette, and crumbled feta cheese, and mix gently.
8. Then transfer the salad to a serving plate. We recommend saving some feta cheese to add at the end, so it stays white and contrasts the rest of the colours.
